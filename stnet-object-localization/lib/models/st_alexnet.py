#TNet: Selective Tuning of Convolutional Networks for Object Localization
#
...
class STAlexNet(nn.Module):
	...
	def load_state_dict_A(self, state_dict_pt, strict=True):
		skip_params = []
	...
	def load_state_dict(self, state_dict_pt, strict=True):
		skip_params = []
		state_dict_self = self.state_dict()
		# print('self: ',state_dict_self)
		# print("to load: ", state_dict_pt)
		state_dict_self_keys = iter(state_dict_self.keys())
		#for (name, param) in enumerate(state_dict_self.items()):
		#	print("name: ", name)
		#	print("key ", param[0])
		#	print("len ", param[1])

		#print("len state: ", len(state_dict_self.keys()))
		#print("len state: ", len(state_dict_pt.items()))
		for i, (name, param) in enumerate(state_dict_pt.items()):
			#print("name: ", name)
			# print("param ", param[0])
			...
			if name in skip_params:
				continue
			state_dict_self_key = next(state_dict_self_keys)
			# print("actual: ",state_dict_self_key)
			if name == 'classifier.1.weight':
				param = param.view(state_dict_self[state_dict_self_key].shape)
			print("namef: ", state_dict_self_key)
			print("paramf: ", name)
			print("paramf: ", param[0])
			print("shape1: ", state_dict_self[state_dict_self_key].shape)
			print("shape11", param.shape)
			while not state_dict_self[state_dict_self_key].shape == param.shape:
				print("shape: ", state_dict_self[state_dict_self_key].shape)
				state_dict_self_key = next(state_dict_self_keys)
			#print("shape1: ", state_dict_self[state_dict_self_key].shape)
			#print("shape11", param.shape)
			state_dict_self[state_dict_self_key].copy_(param)
