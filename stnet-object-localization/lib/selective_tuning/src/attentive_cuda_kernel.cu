    ...
    cudaStream_t stream, int verbose)
{
	...
	/*fprintf(stderr,"CASAEYYYYY____%d", sharedMemSize_1);*/
	if(sharedMemSize_1 > 48*3*1024){
		fprintf(stderr, "*** the required shaaaaaaaaaaaared memory 1: %d at attentive conv layer is above the hardware limit\n",sharedMemSize_1);
		smem_1_oversized = 1;
		...
		if(sharedMemSize_1 > 48*1024){
			fprintf(stderr, "*** NotImplemented:\n\tthe required shareddddd memory 1: %.6f for the second time is above the hardware limit. Reduce (kernel size / channel size) \n",sharedMemSize_1);
			exit(-1);
		}		
	}
